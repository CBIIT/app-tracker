...
describe('ColumnSearchProps', () => {
	...
	it('renders applicant_name highlighter with empty text when value is missing', () => {
		const props = getColumnSearchProps(
			'applicant_name',
			'Applicant',
			'jo',
		...
		const element = props.render(null, { sys_id: '44' });

		expect(element.props.to).toBe(MANAGE_APPLICATION + '44');
		expect(element.props.children.props.searchWords).toEqual(['jo']);
		expect(element.props.children.props.textToHighlight).toBe('');
	});
});
