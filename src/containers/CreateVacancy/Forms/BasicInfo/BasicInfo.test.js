...
describe('BasicInfo', () => {
	...
	it('should render BasicInfo component for new Vacancy', async () => {
		...
		axios.get.mockResolvedValueOnce(mockVacancyOptionsResponse);

		const NewVacancyWrapper = () => {
			const [inputForm] = Form.useForm();
			return (
				<BasicInfo
		...
		render(<NewVacancyWrapper />);

		expect(axios.get).toHaveBeenCalledTimes(1);

		await waitFor(() => {
			const PATSClarification = screen.getByText(
				/The selections made in the fields below will be included in the package sent to/i
			);
