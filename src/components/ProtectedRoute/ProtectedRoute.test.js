...
describe('ProtectedRoute', () => {
    ...
    const renderProtectedRoute = (useOktaAuth = false) =>
        render(
            <MemoryRouter initialEntries={['/protected']}>
                <Routes>
                    <Route
                        path='/protected'
                        element={
                            <ProtectedRoute component={TestComponent} useOktaAuth={useOktaAuth} />
                        }
                    />
                </Routes>
            </MemoryRouter>
    ...
    it('redirects to login when the user is not logged in', () => {
        ...
        expect(window.location.href).toContain(
            '/nav_to.do?uri=%2Fnci-scss.do%23%2Fprotected&glide_sso_id=itrust123'
        );
    });
    ...
    it('redirects to login with Okta SSO when the user is not logged in and useOktaAuth is true', () => {
        ...
        expect(window.location.href).toContain(
            '/nav_to.do?uri=%2Fnci-scss.do%23%2Fprotected&glide_sso_id=okta123'
        );
    });
});
